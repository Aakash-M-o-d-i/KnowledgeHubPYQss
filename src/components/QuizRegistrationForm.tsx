...
interface QuizRegistrationFormProps {
  onSuccess: () => void;
  onCancel: () => void;
  quizDetails: {
    title: string;
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const templateParams = {
        ...
        subject_code: quizDetails.subjectCode,
        category: quizDetails.category,
        message: `Quiz access request for: ${quizDetails.title} (${quizDetails.subjectCode})`
      };
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? "Submitting..." : "Register & Start Quiz"}
          </Button>
        </div>
      </form>
    </Form>
